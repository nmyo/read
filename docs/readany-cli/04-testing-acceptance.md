...
- 单平台打包验收可以使用 `pnpm --filter @readany/cli acceptance:packaged -- --package-source <artifact> --evidence <file>` 采证；该脚本默认只读，记录 version、doctor runtime/distribution、tools list、MCP config、readonly MCP initialize/tools/list 和 skill status。只有显式 `--with-skill-install` 才会写入 agent skill，建议配临时 `--agent-home`。该 evidence 只能补 macOS / Windows / Linux 矩阵中的单平台记录，不能替代真实样本、真实外部 agent 或最终 strict M5。
