...
- 只读查询链路跑通。
- MCP 不展示原始 EPUB/PDF fallback 解析、vector/hybrid RAG、EPUB toc rebuild / export 等未实现工具。
- 测试不依赖真实用户数据。
...
- `tools/list` 没有原始 EPUB/PDF fallback 解析、vector/hybrid RAG、EPUB toc rebuild / export。
- 所有测试通过。
