...
- 如果要在交接或上传后复验 bundle，可执行 `pnpm --filter @readany/cli acceptance:verify-bundle -- --bundle-dir <bundle-dir>`；它会独立检查 `index.json`、record、manifest 和 evidence 的 path/SHA-256/bytes 是否一致，并重跑 strict M5。
