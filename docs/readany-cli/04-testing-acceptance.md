...
- 验收记录、真实样本 evidence、单客户端外部 agent evidence、桌面设置页 evidence 和单平台 packaged evidence 可以使用 `pnpm --filter @readany/cli acceptance:validate -- --record <record.md> --evidence <evidence.json>` 校验结构，`--evidence` 可重复传入多份证据；最终 M5 记录必须加 `--strict-m5` 并传入完整组合证据，用机器闸门阻止“部分通过”、仍有未勾选范围、外部 agent 表格不足两条完整记录、缺少 MCP-backed agent、打包矩阵缺平台记录、缺少桌面设置页证据、manual acceptance 未逐项关闭，或验收记录没有引用真实 evidence 锚点的验收被当作完成。agent / desktop / packaged evidence 是补充证据，strict M5 的 SHA / citation 锚点检查仍需要真实样本 evidence。
