...
- 真实样本 evidence 可以先用 `pnpm --filter @readany/cli acceptance:scaffold -- --evidence <evidence.json> --output <record.md>` 生成 partial 验收草稿；草稿会自动带上 SHA、citation、doctor distribution 和 manual closure 待办。也可以重复传 `--agent-evidence <agent-evidence.json>` 和 `--packaged-evidence <platform-evidence.json>`，把真实外部客户端和单平台 packaged 证据预填进对应表格。缺失外部客户端、draft export、真实安装器验收和跨平台完整矩阵仍要人工补齐，最终仍必须通过 strict 校验。
- 验收记录、真实样本 evidence、单客户端外部 agent evidence 和单平台 packaged evidence 可以使用 `pnpm --filter @readany/cli acceptance:validate -- --record <record.md> --evidence <evidence.json>` 校验结构；最终 M5 记录必须加 `--strict-m5`，用机器闸门阻止“部分通过”、仍有未勾选范围、外部 agent 表格不足两条完整记录、打包矩阵缺平台记录、manual acceptance 未逐项关闭，或验收记录没有引用真实 evidence 锚点的验收被当作完成。agent / packaged evidence 是补充证据，strict M5 的 SHA / citation 锚点检查仍需要真实样本 evidence。
