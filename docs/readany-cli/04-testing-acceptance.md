...
- 真实样本 evidence 可以先用 `pnpm --filter @readany/cli acceptance:scaffold -- --evidence <evidence.json> --output <record.md>` 生成 partial 验收草稿；草稿会自动带上 SHA、citation、doctor distribution 和 manual closure 待办，但仍必须人工补齐并通过 strict 校验。
