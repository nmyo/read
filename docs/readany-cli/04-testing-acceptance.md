...
- 验收记录和真实样本 evidence 可以使用 `pnpm --filter @readany/cli acceptance:validate -- --record <record.md> --evidence <evidence.json>` 校验结构；最终 M5 记录必须加 `--strict-m5`，用机器闸门阻止“部分通过”、仍有未勾选范围、外部 agent 表格不足两条完整记录、打包矩阵缺平台记录、manual acceptance 未逐项关闭，或验收记录没有引用真实 evidence 锚点的验收被当作完成。
