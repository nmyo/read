...
- 单平台打包验收可以使用 `pnpm --filter @readany/cli acceptance:packaged -- --package-source <artifact> --evidence <file>` 采证；该脚本默认只读，记录 version、doctor runtime/distribution、tools list、MCP config、readonly MCP initialize/tools/list 和 skill status。只有显式 `--with-skill-install` 才会写入 agent skill，建议配临时 `--agent-home`；只有显式 `--draft-export --book <epub-book-id> --export-dir <dir>` 才会创建 draft、validate、export、检查导出 EPUB 结构并清理 draft。该 evidence 只能补 macOS / Windows / Linux 矩阵中的单平台记录，不能替代真实样本、真实外部 agent 或最终 strict M5。
- 真实样本 evidence 可以先用 `pnpm --filter @readany/cli acceptance:scaffold -- --evidence <evidence.json> --output <record.md>` 生成 partial 验收草稿；草稿会自动带上 SHA、citation、doctor distribution 和 manual closure 待办。也可以重复传 `--packaged-evidence <platform-evidence.json>`，把单平台 packaged 证据预填进 macOS / Windows / Linux 打包矩阵。该矩阵行仍会标记为 partial，draft export 和真实安装器验收必须人工补齐，最终仍必须通过 strict 校验。
- 验收记录、真实样本 evidence 和单平台 packaged evidence 可以使用 `pnpm --filter @readany/cli acceptance:validate -- --record <record.md> --evidence <evidence.json>` 校验结构；最终 M5 记录必须加 `--strict-m5`，用机器闸门阻止“部分通过”、仍有未勾选范围、外部 agent 表格不足两条完整记录、打包矩阵缺平台记录、manual acceptance 未逐项关闭，或验收记录没有引用真实 evidence 锚点的验收被当作完成。packaged evidence 是补充证据，strict M5 的 SHA / citation 锚点检查仍需要真实样本 evidence。
