...
- 真实样本验收可以使用 `pnpm --filter @readany/cli acceptance:real -- --book <book-id> --rag-query <query> --evidence <file>` 采证；该脚本默认只读，只有显式 `--draft-export --export-dir <dir>` 才会创建 EPUB draft、validate、export、inspect 导出 EPUB，并默认 discard draft 清理验收工作区，不替代真实外部 agent 和打包产物验收。
