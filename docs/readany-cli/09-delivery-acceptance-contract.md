...
- `epub.inspect` 当前是只读结构检查，不代表已经支持 toc rebuild / validate / export。
- `epub.draft.create` 当前只创建受控 draft workspace，不代表已经支持校验或导出。
...
- `epub.history` 当前只读取 draft operation history，不做 diff、不撤销、不修改文件。
...
- `epub.history` 当前只读取 draft operation history，不生成 diff、不执行 undo。
- EPUB toc rebuild、validate、export 尚未完成前不会出现在 MCP `tools/list`。
