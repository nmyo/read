...
- MCP 只暴露真实实现的工具；当前除 `epub.inspect`、`epub.draft.create`、`epub.chapter.read`、`epub.chapter.patch`、`epub.metadata.patch` 需要 editor profile 外，其余 MCP 工具都走 readonly profile。
...
- MCP 当前只暴露 `books.list`、`books.search`、`books.get`、`chapters.list`、`chapters.get`、`notes.search`、`highlights.search`、`rag.search`、`epub.inspect`、`epub.draft.create`、`epub.chapter.read`、`epub.chapter.patch`、`epub.metadata.patch`。
