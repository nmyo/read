...
- MCP 只暴露真实实现的工具；当前除 `epub.inspect`、`epub.draft.create`、`epub.chapter.read`、`epub.chapter.patch`、`epub.metadata.patch`、`epub.history`、`epub.diff` 需要 editor profile 外，其余 MCP 工具都走 readonly profile。
