...
- MCP 当前只暴露 `books.list`、`books.search`、`books.get`、`notes.search`、`highlights.search`。
...
只读元数据不足以支撑外部 AI 深度阅读。M2 需要开放真实章节内容和 RAG 检索，但仍保持 readonly。
...
- MCP: `chapters.list`、`chapters.get`、`rag.search`
- 分页、范围读取、引用定位
