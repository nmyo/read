...
- MCP 当前只暴露 `books.list`、`books.search`、`books.get`、`chapters.list`、`chapters.get`、`notes.search`、`highlights.search`、`rag.search`。
...
其中 indexed chunks 章节视图和 `rag.search` 的 BM25 over chunks 能力已经提前落地；M2 剩余重点是原始 EPUB/PDF fallback 章节解析、上下文资源，以及 vector / hybrid 模式的完整验收。
...
只读元数据不足以支撑外部 AI 深度阅读。M2 需要开放真实章节内容和完整 RAG 检索，但仍保持 readonly。
...
- MCP: `chapters.list`、`chapters.get`，以及 `rag.search` 的完整模式
- 分页、范围读取、引用定位
