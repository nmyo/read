...
```text
外部 AI 可以发现 ReadAny，可以列书、搜书、读书籍元数据、搜笔记、高亮和已索引 chunks。
...
- MCP 当前只暴露 `books.list`、`books.search`、`books.get`、`notes.search`、`highlights.search`、`rag.search`。
...
其中 `rag.search` 的 BM25 over chunks 能力已经提前落地；M2 剩余重点是章节内容、上下文资源，以及 vector / hybrid 模式的完整验收。
