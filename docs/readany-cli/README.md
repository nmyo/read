...
- 用户精排入口不放在设置页；书籍详情页已接入创建精排草稿，并可打开首版 EPUB draft 工作区查看 history、entry-level diff 和 validate 结果；后续完整 draft 工作区继续承接章节编辑、元数据编辑、undo/discard 和 export；设置页只负责接入和权限管理。
