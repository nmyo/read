...
- 已支持归档包复验：`pnpm --filter @readany/cli acceptance:verify-bundle -- --bundle-dir <bundle-dir>` 会独立检查 bundle 目录里的 `index.json`、`record.md`、`manifest.json` 和 evidence 文件是否彼此匹配，并重跑 strict M5 复验，适合交接、上传后抽检或 CI 二次校验。
