...
- MCP 当前只暴露真实实现的工具：`books.list`、`books.search`、`books.get`、`chapters.list`、`chapters.get`、`notes.search`、`highlights.search`、`rag.search`、`epub.inspect`、`epub.draft.create`、`epub.chapter.read`、`epub.chapter.patch`、`epub.metadata.patch`。
...
- EPUB toc rebuild / validate / export。`epub.inspect` 只是只读结构检查；`epub.draft.create` 只创建受控 draft；`epub.chapter.read` 只读取 draft 章节文本；`epub.chapter.patch` 只替换 draft 内单章 XHTML；`epub.metadata.patch` 只修改 draft OPF metadata，不重建目录或导出文件。
