...
- 已支持归档包复验：`pnpm --filter @readany/cli acceptance:verify-bundle -- --bundle-dir <bundle-dir>` 会独立检查 bundle 目录里的 `index.json`、`record.md`、`manifest.json` 和 evidence 文件是否彼此匹配，适合交接、上传后抽检或 CI 二次校验。
- 已支持一键最终归档：`pnpm --filter @readany/cli acceptance:assemble -- --record <record.md> --evidence <evidence.json> ... --output-dir <bundle-dir>` 会先执行 strict M5 校验，再在 bundle 目录里生成 `final-manifest.json`，随后复制出最终 `record.md`、`manifest.json`、`index.json` 和全部 evidence，并自动跑一遍 `acceptance:verify-bundle`，适合作为发布前最后一步。
