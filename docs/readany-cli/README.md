...
- 已支持单平台打包验收采证：`pnpm --filter @readany/cli acceptance:packaged -- --package-source <artifact> --evidence <file>` 会对安装后 CLI 或指定 `--cli` 执行 version、doctor、tools list、MCP config、readonly MCP initialize/tools/list、skill status，并记录 runtime/distribution、MCP safety metadata 和平台来源；`--with-skill-install` 才会执行 skill install/status/uninstall，建议配临时 `--agent-home`。该证据只补单平台矩阵，不替代真实样本、真实外部 agent 或最终 strict M5 记录。
- 已支持验收记录草稿生成：`pnpm --filter @readany/cli acceptance:scaffold -- --evidence <evidence.json> --output <record.md>` 会把真实 evidence 中的 SHA、citation、doctor distribution 和人工补证项写入 partial 记录草稿，减少手工填表漏锚点；如果再传 `--packaged-evidence <platform.json>`，还会把单平台 packaged 证据预填到打包矩阵里，但不会把 macOS / Windows / Linux 三平台都自动判成通过。
- 已支持验收记录校验脚本：`pnpm --filter @readany/cli acceptance:validate -- --record <record.md> --evidence <evidence.json>` 可检查验收记录和真实样本 evidence 结构；最终 M5 记录需要加 `--strict-m5`，阻止 partial record 被误当作完成。
