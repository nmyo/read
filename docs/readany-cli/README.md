...
- 已支持真实样本验收辅助脚本：`pnpm --filter @readany/cli acceptance:real -- --book <book-id> --rag-query <query> --evidence <file>` 会使用 built CLI 对指定真实书库样本执行 books/chapter/RAG/knowledge/context/audit 检查，并在 evidence JSON 中记录运行环境、样本书文件路径、字节数、SHA-256 和仍需人工验收的 M5 清单；默认只读，只有显式 `--draft-export --export-dir <dir>` 才会创建 EPUB draft、validate、export、inspect 导出 EPUB，并默认 discard draft 清理工作区。
