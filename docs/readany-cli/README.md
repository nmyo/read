...
- 已支持 `doctor`、`install`、`uninstall`、`skill install/uninstall/status`；`doctor --json` 会报告 Node runtime 和 `better-sqlite3` 可解析性，便于区分“管理命令可用”和“书库/MCP 数据命令可用”。
...
- 已支持真实样本验收辅助脚本：`pnpm --filter @readany/cli acceptance:real -- --book <book-id> --rag-query <query> --evidence <file>` 会使用 built CLI 对指定真实书库样本执行 books/chapter/RAG/knowledge/context/audit 检查；默认只读，只有显式 `--draft-export --export-dir <dir>` 才会创建 EPUB draft、validate、export、inspect 导出 EPUB，并默认 discard draft 清理工作区。
