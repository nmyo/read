...
- 已支持最终验收包目录：`pnpm --filter @readany/cli acceptance:bundle -- --record <record.md> --manifest <manifest.json> --evidence <evidence.json> ... --output-dir <bundle-dir>` 会把最终 record、manifest 和所有 evidence 复制到一个可归档目录，并生成带 source/target/SHA-256/bytes 清单的 `index.json`。
- 已支持一键最终归档：`pnpm --filter @readany/cli acceptance:assemble -- --record <record.md> --evidence <evidence.json> ... --output-dir <bundle-dir>` 会先执行 strict M5 校验，再在 bundle 目录里生成 `final-manifest.json`，随后复制出最终 `record.md`、`manifest.json`、`index.json` 和全部 evidence，适合作为发布前最后一步。
