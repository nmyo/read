...
- Draft export audit：`packages/cli/src/commands.test.ts` 在真实 fixture EPUB draft export 流程中验证成功导出和覆盖失败都会进入 audit 摘要，且 audit 不包含 draft id 或输出路径。
...
- audit 不含完整正文 / 密钥 / 同步凭证：CLI/MCP audit tests 覆盖摘要读取；CLI fixture draft export 流程覆盖导出审计摘要不泄漏 draft id / output path。真实外部 agent 和打包产物链路仍需补验收记录。
