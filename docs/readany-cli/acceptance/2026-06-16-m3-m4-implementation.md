...
- Runtime smoke：`packages/cli/src/build-smoke.test.ts` 阻断 `better-sqlite3` 加载后验证 `--version`、`doctor --json`、`tools list --json`、`skill status/install/uninstall --json` 可以运行；构建后 stdio MCP 可响应 `initialize`、`tools/list`、readonly `books.list`，并拒绝 readonly `epub.export`。
