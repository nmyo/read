...
- External agent smoke：`pnpm --filter @readany/cli smoke:agent` 通过，使用 built CLI stdio MCP 跑 readonly 发现/搜索、readonly 写入拒绝、editor draft 批量章节修改和 toc rebuild、publisher validate/export、MCP audit 导出记录、原 EPUB hash 不变，以及导出 EPUB 重新入库后的 inspect / chapter read 检查。
