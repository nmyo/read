...
- Release matrix preflight backlog：本地已提供 `pnpm cli:preflight`，顺序执行 CLI check/test/build、built CLI external agent smoke、Tauri CLI bridge tests 和 `cargo check`；建议 `.github/workflows/release.yml` 的 macOS / Windows / Linux Tauri job 在 `tauri-action` 打包前复用该入口。当前 Codex OAuth token 没有 GitHub `workflow` scope，无法直接推送 workflow 文件改动，需具备 workflow 权限的提交者补上。
