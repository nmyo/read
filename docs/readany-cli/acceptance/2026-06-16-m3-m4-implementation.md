...
- MCP config generation：`readany mcp config --profile readonly --json` / `--profile publisher` 覆盖在 `packages/cli/src/commands.test.ts` 和 built CLI smoke 中，非法 profile 返回错误；该命令只生成外部 agent 配置片段，不出现在 MCP `tools/list`。
