...
18. `readany mcp serve --profile readonly` 的 stdio JSON-RPC 最小协议。
19. 桌面客户端设置页里的外部 AI 访问入口与 readonly MCP 配置复制。
...
原始 EPUB/PDF fallback 章节解析、vector/hybrid RAG、EPUB toc rebuild / export、同步、备份在后续阶段接入。`epub.inspect` 只是只读结构检查；`epub.draft.create` 只复制原 EPUB 到受控 draft workspace 并写入 manifest/history；`epub.draft.discard` 只标记 draft inactive；`epub.chapter.read` 只读取 draft 章节文本；`epub.chapter.patch` 只替换 draft 内单个 XHTML 章节，不修改原书；`epub.metadata.patch` 只修改 draft OPF metadata；`epub.history` 只读取 draft operation history；`epub.diff` 只比较 source/draft EPUB entry 的 hash 和 size，不生成内容级 diff、不执行 undo；`epub.validate` 只做 active draft 结构和资源引用校验，不自动修改、不导出。其余 EPUB 写入、export 工具接入前不允许伪造实现，不允许在 MCP `tools/list` 里提前出现。
