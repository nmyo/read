...
19. `readany mcp serve --profile readonly` 的 stdio JSON-RPC 最小协议。
20. 桌面客户端设置页里的外部 AI 访问入口与 readonly MCP 配置复制。
