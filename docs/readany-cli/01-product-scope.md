...
11. `readany mcp serve --profile readonly` 的 stdio JSON-RPC 最小协议。
...
原始 EPUB/PDF fallback 章节解析、vector/hybrid RAG、EPUB draft patch、导出、同步、备份在后续阶段接入。`epub.inspect` 只是只读结构检查；其余 EPUB 写入、draft、export 工具接入前不允许伪造实现，不允许在 MCP `tools/list` 里提前出现。
