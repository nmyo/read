...
8. `readany chapters list`、`readany chapter get` 的章节视图：优先 indexed chunks；未索引 EPUB fallback 到真实 EPUB spine/manifest。
