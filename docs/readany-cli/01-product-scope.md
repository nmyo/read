...
8. `readany rag search --book <book-id>` 的 BM25 chunk 检索。
9. `readany mcp serve --profile readonly` 的 stdio JSON-RPC 最小协议。
...
章节正文、vector/hybrid RAG、EPUB 精排、导出、同步、备份在后续阶段接入。接入前不允许伪造工具，不允许在 MCP `tools/list` 里提前出现。
