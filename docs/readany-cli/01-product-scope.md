...
10. `readany mcp serve --profile readonly` 的 stdio JSON-RPC 最小协议。
...
原始 EPUB/PDF fallback 章节解析、vector/hybrid RAG、EPUB 精排、导出、同步、备份在后续阶段接入。接入前不允许伪造工具，不允许在 MCP `tools/list` 里提前出现。
