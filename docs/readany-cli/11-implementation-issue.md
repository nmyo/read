...
- 用户 draft 工作区完整 UI 已经完成。书籍详情页已经提供精排草稿入口并可通过受限 CLI bridge 创建 draft；工作区已能查看 history、entry-level diff 和 validate 结果，并能通过受限 action 执行章节 XHTML 读取/保存、toc rebuild、undo、discard，但完整元数据编辑 / export 工作区仍需继续。
