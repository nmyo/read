...
- 用户 draft 工作区完整 UI 已经完成。书籍详情页已经提供精排草稿入口并可通过受限 CLI bridge 创建 draft；工作区已能查看 history、entry-level diff 和 validate 结果，并能通过受限 action 执行 toc rebuild、undo、discard，但完整章节编辑 / 元数据编辑 / export 工作区仍需继续。
